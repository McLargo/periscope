...
class LegendasTV(SubtitleDatabase.SubtitleDB):
    ...
    def __init__(self, config, cache_folder_path):
        ...
        self.tvshowRegex = re.compile(
            "(?P<show>.*)S(?P<season>[0-9]{2})E(?P<episode>[0-9]{2}).(?P<teams>.*)",
            re.IGNORECASE,
        )
        self.tvshowRegex2 = re.compile(
            "(?P<show>.*).(?P<season>[0-9]{1,2})x(?P<episode>[0-9]{1,2}).(?P<teams>.*)",
            re.IGNORECASE,
        )
        self.movieRegex = re.compile(
            "(?P<movie>.*)[\_\.|\[|\(| ]{1}(?P<year>(?:(?:19|20)[0-9]{2}))(?P<teams>.*)",
            re.IGNORECASE,
        )
        self.user = None
        self.password = None
    ...
    def process(self, filepath, langs):
        ...
        if not self.user or self.user == "":
            log.error(
                "LegendasTV requires a personnal username/password. Set one up in your ~/.config/periscope/config file"
            )
            return []
        ...
        if dados["type"] == "tvshow":
            subtitles = self.LegendasTVSeries(
                filepath,
                dados["name"],
                str(dados["season"]),
                str(dados["episode"]),
            ...
            subtitles = self.LegendasTVMovies(
                filepath, dados["name"], dados["year"], langs
            )
        else:
    ...
    def extractFile(self, fname, extract_path, extractedFiles=[]):
        ...
        for root, dirs, files in os.walk(
            extract_path.encode(fs_encoding), topdown=False
        ):
            for f in files:
                ext = os.path.splitext(f)[1].lower()
                if ext in [".zip", ".rar"]:
                    self.extractFile(
                        os.path.join(root, f), extract_path, extractedFiles
                    )
    ...
    def downloadFile(self, url, srtfilename):
        ...
        extract_path = os.path.join(
            srtfilename.replace(self.getFileName(srtfilename), ""), str(url)
        )
        ...
        log.debug(
            "Renaming [%s] to [%s] "
            % (os.path.join(extract_path, legenda_retorno), srtfilename)
        )
    ...
    def LegendasTVMovies(self, file_original_path, title, year, langs):
        ...
        request = urllib2.Request(
            self.url + "/index.php?opcao=buscarlegenda", search_data
        )
        ...
        if (
            response.__contains__("Nenhuma legenda foi encontrada")
            and search_string != title
        ):
            log.debug(
                "No subtitles found using the original file name, using title instead."
            )
        ...
        for x in range(pages):
            if x:
                html = urllib2.urlopen(
                    self.url + "/index.php?opcao=buscarlegenda&pagina=" + str(x + 1)
                ).read()
        ...
        for td in td_results:
            ...
            for span in span_results:
                ...
                ltv_lang = re.findall(
                    "images/flag_([^.]*).gif",
                    span.findAll("td")[4].contents[0].attrs[0][1],
                )
    ...
    def LegendasTVSeries(self, file_original_path, tvshow, season, episode, langs):
        ...
        for span in span_results:
            ...
            if reResult:
                ltv_title = reResult[0]
            else:
                ltv_title = self.CleanLTVTitle(td.contents[2])

            reResult = re.findall(
                "(.*) - ([0-9]*)", self.CleanLTVTitle(td.contents[0].contents[0])
            )
            if reResult:
                ltv_original_title, ltv_season = reResult[0]
            else:
            ...
            if not ltv_season:
                ...
                if reResult:
                    ltv_season = re.sub("^0", "", reResult[0])

            if not ltv_season:
                continue

            # This is the download ID for the subtitle.
            download_id = re.search("[a-z0-9]{32}", td.parent.parent.attrs[1][1]).group(
                0
            )
            ...
            # and convert it to the OpenSubtitles format.
            ltv_lang = re.findall(
                "images/flag_([^.]*).gif", span.findAll("td")[4].contents[0].attrs[0][1]
            )
            if ltv_lang:
                ltv_lang = ltv_lang[0]
            if ltv_lang == "br":
            ...
            if int(ltv_season) == int(season):
                ...
                if (
                    re.findall("^%s" % (tvshow), ltv_original_title)
                    or self.comparetitle(ltv_title, tvshow)
                    or self.comparetitle(ltv_original_title, original_tvshow)
                ):
                    sub1.append(SubtitleResult)
                else:
                    reResult = re.findall("[Ss][0-9]+[Ee]([0-9]+)", release)
                    if reResult:
                        LTVEpisode = re.sub("^0", "", reResult[0])
                    else:
                        LTVEpisode = 0
    ...
    def comparetitle(self, a, b):
        if (
            (a == b)
            or (self.noarticle(a) == self.noarticle(b))
            or (a == self.noarticle(b))
            or (self.noarticle(a) == b)
            or (a == self.shiftarticle(b))
            or (self.shiftarticle(a) == b)
        ):
            return 1
        else:
            return 0
