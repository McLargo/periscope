...
class Subtitulos(SubtitleDatabase.SubtitleDB):
    ...
    def __init__(self, config, cache_folder_path):
        ...
        self.release_pattern = re.compile(
            "Versi&oacute;n (.+) ([0-9]+).([0-9])+ megabytes"
        )
    ...
    def query(self, name, season, episode, teams, langs=None):
        ...
        for subs in soup("div", {"id": "version"}):
            ...
            subteams = (
                self.release_pattern.search("%s" % version.contents[1]).group(1).lower()
            )
            teams = set(teams)
            ...
            for lang_html in nexts:
                ...
                link = statusLI.findNext("span", {"class": "descargar green"}).find(
                    "a"
                )["href"]
                if (
                    status == "Completado"
                    and subteams.issubset(teams)
                    and (not langs or lang in langs)
                ):
                    result = {}
                    result["release"] = "%s.S%.2dE%.2d.%s" % (
                        name.replace("-", ".").title(),
                        int(season),
                        int(episode),
                        ".".join(subteams),
