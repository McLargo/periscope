...
class OpenSubtitles(SubtitleDatabase.SubtitleDB):
    ...
    def process(self, filepath, langs):
        ...
        if os.path.isfile(filepath):
            ...
            return self.query(
                moviehash=filehash, langs=langs, bytesize=size, filename=fname
            )
        else:
            fname = self.getFileName(filepath)
    ...
    def query(self, filename, imdbID=None, moviehash=None, bytesize=None, langs=None):
        ...
        if langs:
            search["sublanguageid"] = ",".join(
                [self.getLanguage(lang) for lang in langs]
            )
        if len(search) == 0:
    ...
    def get_results(self, token, search):
        ...
        if results["data"]:
            ...
            for r in sorted(results["data"], self.sort_by_moviereleasename):
                ...
                if search.has_key(
                    "query"
                ):  # We are using the guessed file name, let's remove some results
                    if r["MovieReleaseName"].startswith(self.filename):
                        sublinks.append(result)
                    else:
                        log.debug(
                            "Removing %s because release '%s' has not right start %s"
                            % (result["release"], r["MovieReleaseName"], self.filename)
                        )
                else:
                    sublinks.append(result)
        return sublinks
    ...
    def sort_by_moviereleasename(self, x, y):
        ...
        xmatch = x["MovieReleaseName"] and (
            x["MovieReleaseName"].find(self.filename) > -1
            or self.filename.find(x["MovieReleaseName"]) > -1
        )
        ymatch = y["MovieReleaseName"] and (
            y["MovieReleaseName"].find(self.filename) > -1
            or self.filename.find(y["MovieReleaseName"]) > -1
        )
        # print "analyzing %s and %s = %s and %s" %(x['MovieReleaseName'], y['MovieReleaseName'], xmatch, ymatch)
        if xmatch and ymatch:
            if x["MovieReleaseName"] == self.filename or x[
                "MovieReleaseName"
            ].startswith(self.filename):
                return -1
            return 0
