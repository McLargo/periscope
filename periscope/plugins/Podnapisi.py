...
class Podnapisi(SubtitleDatabase.SubtitleDB):
    ...
    def __init__(self, config, cache_folder_path):
        ...
        self.host = "https://podnapisi.net"
        self.search = "/subtitles/search?"
    ...
    def query(self, token, langs=None):
        ...
        params = {"keywords": token}
        if langs and len(langs) == 1:
            params["language"] = langs[0]
        else:
            params["language"] = "en"

        searchurl = self.host + self.search + urllib.urlencode(params)
        content = self.downloadContent(searchurl, 10, use_request=True)
        if content is None:
            return sublinks
        ...
        for subs in soup("tr", {"class": "a"}) + soup("tr", {"class": "b"}):
            ...
            if (
                guessedData["type"] == "tvshow"
                and guessedData["season"] == int(details[0].text)
                and guessedData["episode"] == int(details[1].text)
            ):
