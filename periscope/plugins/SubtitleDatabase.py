...
class SubtitleDB(object):
    ...
    def __init__(self, langs, revertlangs=None):
        ...
        if revertlangs:
            ...
            self.langs = dict(
                map(lambda item: (item[1], item[0]), self.revertlangs.items())
            )
        ...
        self.movieRegex = re.compile(
            "(?P<movie>.*)[\.|\[|\(| ]{1}(?P<year>(?:(?:19|20)[0-9]{2}))(?P<teams>.*)",
            re.IGNORECASE,
        )
    ...
    def searchInThread(self, queue, filename, langs):
        ...
        try:
            ...
            log.info(
                "%s writing %s items to queue" % (self.__class__.__name__, len(subs))
            )
        except:
        ...
        queue.put(
            subs, True
        )  # Each plugin must write as the caller periscopy.py waits for an result on the queue
        ...
        if zipfile.is_zipfile(zipfilename):
            ...
            for el in zf.infolist():
                if el.orig_filename.rsplit(".", 1)[1] in ("srt", "sub", "txt"):
                    outfile = open(
                        srtbasefilename + "." + el.orig_filename.rsplit(".", 1)[1], "wb"
                    )
    ...
    def downloadContent(self, url, timeout=None, use_request=False):
    ...
    def getLG(self, language):
        ...
        try:
            ...
            log.warn(
                "Ooops, you found a missing language in the config file of %s: %s. Send a bug report to have it added."
    ...
    def getLanguage(self, lg):
        ...
        try:
            ...
            log.warn(
                "Ooops, you found a missing language in the config file of %s: %s. Send a bug report to have it added."
                % (self.__class__.__name__, lg)
            )

    def query(self, token):
        raise TypeError(
            "%s has not implemented method '%s'"
