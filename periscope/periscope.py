...
class Periscope:
    ...
    def listExistingPlugins(self):
        ...
        return map(
            lambda x: x.__name__, plugins.SubtitleDatabase.SubtitleDB.__subclasses__()
        )
        ...
        for name in self.pluginNames:
            try:
                ...
                thread = threading.Thread(
                    target=plugin.searchInThread, args=(q, filename, langs)
                )
                thread.start()
            except ImportError:
    ...
    def selectBestSubtitle(self, subtitles, langs=["en"], interactive=False):
        ...
        if not interactive:
            ...
            while not sub:
                try:
                    sub = interactive_subtitles[
                        int(raw_input("Please select a subtitle: "))
                    ]
                    if sub:
                        return sub
                except IndexError:
    ...
    def attemptDownloadSubtitle(self, subtitles, langs, interactive=False):
        ...
        if subtitle:
            ...
            try:
                ...
                log.warn(
                    "Subtitle %s could not be downloaded, trying the next on the list"
                    % subtitle["link"]
                )
                ...
                log.error(
                    "Type[%s], Message [%s], Traceback[%s]" % (etype, evalue, etb)
                )
    ...
    def __orderSubtitles__(self, subs):
        ...
        try:
            ...
            subtitles = defaultdict(
                list
            )  # Order matters (order of plugin and result from plugins)
            for s in subs:
