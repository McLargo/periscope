...
def main():
    ...
    parser = OptionParser(
        "usage: %prog [options] file1 file2", version=periscope.VERSION
    )
    parser.add_option(
        "-l",
        "--language",
        action="append",
        dest="langs",
        help="wanted language (ISO 639-1 two chars) for the subtitles "
        "(fr, en, ja, ...). If none is specified will download "
        "a subtitle in any language. "
        "This option can be used multiple times like"
        "%prog -l fr -l en file1 will try to download in french "
        "and then in english if no french subtitles are found.",
    )
    parser.add_option(
        "-f",
        "--force",
        action="store_true",
        dest="force_download",
        help="force download of a subtitle even there is already one present",
    )
    parser.add_option(
        "-q",
        "--query",
    ...
    parser.add_option(
        ...
        action="store_true",
        dest="quiet",
        help="run in quiet mode (only show warn and error messages)",
    )
    parser.add_option(
        "--debug",
        action="store_true",
    ...
    if not options.cache_folder:
        try:
            ...
            if home == "~":
                log.error(
                    "Could not generate a cache folder at the home location using XDG (freedesktop). You must specify a --cache-config folder where the cache and config will be located (always use the same folder)."
                )
                exit()
    ...
    if options.show_active_plugins:
        ...
        print("Active plugins: {}".format(plugins))
        exit()

    if options.show_plugins:
        plugins = periscope_client.listExistingPlugins()
        print("All plugins: {}".format(plugins))
        exit()
    ...
    if os.path.isdir(entry):
        ...
        if mimetype in SUPPORTED_FORMATS:
            ...
            if options.force_download or not (
                os.path.exists(basepath + ".srt") or os.path.exists(basepath + ".sub")
            ):
            ...
            log.info(
                "%s mimetype is '%s' which is not a supported video format (%s)"
                % (entry, mimetype, SUPPORTED_FORMATS)
            )
    return files
